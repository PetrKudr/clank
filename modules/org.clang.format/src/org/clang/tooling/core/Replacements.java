...
import org.clang.format.impl.MergedReplacement;
import org.clang.format.impl.ReplacementStatic;
...
public class Replacements implements Destructors.ClassWithDestructor {
//private:
  ...
  public Error add(/*const */Replacement /*&*/ R) {
    ...
    BiFunction<Replacement, Replacement, Boolean> Overlap
        = (/*const Replacement &*/R1, /*const Replacement &*/ R2) -> {
          return $c$.clean($c$.track(new Range(R1.getOffset(), R1.getLength())))
              .overlapsWith($c$.track(new Range(R2.getOffset(), R2.getLength())));
        };
...
//    Result.insert(Merged.asReplacement());
  ...
  public vector<Range> getAffectedRanges() /*const*/ {
    ...
    return ReplacementStatic.combineAndSortRanges(ChangedRanges);
  }
